...
public class NBTDataHandler {
	...
	public static void printNBTTagContentsForDimension(int dimensionID){
		for(VillagerData data : villagerData){
			if(data.getDimensionID() == dimensionID){
				LogManager.getLogger().log(Level.INFO, "VillagerData for dimension " + dimensionID + " contains the following data:");
				for(String key : data.getData().getKeySet()){
					LogManager.getLogger().log(Level.INFO, "Key: " + key + " = " + data.getData().getInteger(key));
				}
			}
		}
	}
